%{
# pl_pr.Deconvsang2 : deconvolution with Sang's algorithm
-> pl_pr.Ext2  # I will rather put all cells together, it is much faster
-----


%}

classdef Deconvsang2 < dj.Computed

	methods(Access=protected)

		function makeTuples(self, key)
            
            rel = pl_pr.Deconvsang&key;
            if rel.count>0
                dat = fetch(rel,'*');
                ...
                for i = 1 : length(fldnames)
                    tuple.(fldnames{i}) = dat.(fldnames{i});                  
                end
                %self.insert(tuple)
                makeTuples(pl_pr.Deconvsang2ROI,tuple);

            end
            return;
                
            
            %% I have to revise in the future 10-02-2018
            % GCaMP6s : tau=0.85
            % GcaMP7f : tau =0.2;
            [gene,taus] = fetchn(common.Virus&key,'gene','decaytime')
            for i =1 : length(gene)
                if strfind(gene{i},'GCaMP')
                    tau = taus(i);
                    break;
                end
            end
            ...
            par = struct('dt',[],'dff_fac',[],'dffn_fac',0,...
                ...
                'tau',tau,'chunk',1000,'ctm_scale',0.7,...
