classdef contgain 
    ...
    methods
        ...
        % function cinx = select_cell_contoristat(self, baseframe, respframe, respthr)
            ...
            condition ='cond_idx';
            y0 = get_respsort_evt(self, baseframe, condition);
            yP = get_respsort_evt(self, respframe, condition);
            p = zeros(1,size(yP,3));
            for i = 1:size(yP,3)
        ...
        function [y, uevt] = get_respsort_evt(self, inxfr, condition)
            X1 = squeeze(mean(self.X(:,inxfr,:),2));
            ...
            y = NaN*ones(maxtrial,length(uevt), size(X1,2));
            for i = 1: length(uevt)
                Nt = size(Xe{i},1);
                y(1:Nt,i,:)=Xe{i};
            end
        ...
        function [mori, seori, ori] = get_oritun(self,inx_cell,varargin)
            ...
            % condtion(varargin)
            % e.g., [mori, seori, ori] = get_oritun(self,inx_cell,'contrast',40)
        ...
        function [SC, NC, uevt] = get_corr(self,inxcell,inxf,evtname)
            % function [SC, NC, ucid] = get_corr(self,inxcell,inxf,evtname)
            % SC: signal correlation
            % NC: noise correlation

            [X1, uevt] = get_respsort_evt(self, inxf, evtname);
            nc = size(X1,2);
            ncell = length(inxcell);
            NC = NaN(ncell,ncell,nc);            
            
            for i0 = 1 : nc
                Xi = squeeze(X1(:,i0,inxcell));                
                NC(:,:,i0)=corr(Xi);
            end
            mX = squeeze(nanmean(X1(:,:,inxcell),1));
            SC = corr(mX);
        end
